...
const DesktopTabs: React.FC<DesktopTabsProps> = ({ 
  ...
  return (
    <div className="hidden sm:block">
      ...
      >
        <div className="flex justify-center mb-12 w-full overflow-hidden">
          <TabsList className="flex overflow-x-auto justify-start sm:justify-center px-2 gap-4 bg-transparent">
            {segments.map(segment => (
              <TabsTrigger 
                key={segment.id || `tab-${Math.random().toString(36).substr(2, 9)}`} 
                value={segment.id} 
                className="px-3 sm:px-4 py-2 sm:py-4 flex-shrink-0"
              >
                <div className="flex flex-col items-center">
                  <span className="flex mb-2">
                    {segment.icon}
                  </span>
                  <span className="text-sm sm:text-base mt-1 font-medium whitespace-nowrap">
                    {segment.name}
                  </span>
                </div>
              </TabsTrigger>
