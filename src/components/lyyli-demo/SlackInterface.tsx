...
const SlackInterface: React.FC<SlackInterfaceProps> = ({ animationPhase }) => {
  ...
  // Pre-fetch all needed translations with fallbacks
  const aiAssistant = safeT('demo.slackIntegration.aiAssistant', { fallback: 'Lyyli AI' });
  const slackTitle = safeT('demo.slackIntegration.title', { fallback: 'Slack Integration' });
  const slackDescription = safeT('demo.slackIntegration.description', { 
    fallback: 'Use Lyyli directly from Slack - no need to switch applications' 
  });
  const contentHeader = safeT('demo.generatedContent.header', { 
    fallback: '🌱 NEW SUSTAINABILITY INITIATIVE - ACTIONS START NEXT WEEK' 
  });
  const commandPlaceholder = safeT('demo.slackIntegration.commandPlaceholder', { 
    fallback: 'type /lyyli [your task]' 
  });
  const teamsFallback = safeT('demo.slackIntegration.teamsFallback', { 
    fallback: 'Also works with Microsoft Teams' 
  });
  const teamReaction = safeT('demo.generatedContent.teamReaction', { 
    fallback: '15 people reacted to this message 👍' 
  });
  const readyToReplaceTitle = safeT('demo.readyToReplaceTitle', { 
    fallback: 'Ready to replace time-consuming communication tasks?' 
  });
  const readyToReplaceButton = safeT('demo.readyToReplaceButton', { 
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="bg-[#222529] text-white p-4 flex-1 overflow-y-auto">
          ...
          <div className="flex items-start mb-1">
            ...
            <div className="ml-2">
              <div className="flex items-center">
                <span className="font-medium">{aiAssistant}</span>
                <span className="ml-2 text-xs text-gray-400">11:45 AM</span>
              </div>
              <div className="mt-1">
                <p className="font-medium">✨ {slackTitle} 🚀</p>
                <p className="mt-2">{slackDescription} 📈</p>
                <p className="mt-2">{contentHeader}</p>
                <ul className="list-disc ml-6 mt-1">
                  <li>{commandPlaceholder}</li>
                  <li>{teamsFallback}</li>
                </ul>
                ...
                <p className="mt-2">{teamReaction}</p>
              </div>
        ...
        {animationPhase >= 8 && (
          <div className="bg-gradient-to-r from-primary/80 to-primary p-4 text-white text-center animate-fade-in">
            <h3 className="text-lg font-semibold mb-2">{readyToReplaceTitle}</h3>
            <p>{readyToReplaceButton}</p>
          </div>
        )}
      </div>
    </ErrorBoundary>
